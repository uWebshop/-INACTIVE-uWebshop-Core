...
namespace uWebshop.Domain.Services
{
    internal class CatalogUrlResolvingService : ICatalogUrlResolvingService
    {
        ...
        public IEnumerable<ICategory> GetCategoryPathFromUrlName(string categoryUrlName)
        {
            ...
            var categories = _categoryService.GetAll(_storeService.GetCurrentLocalization()).ToList();
        ...
        private class CategoryTreeWalker
        {
            ...
            {
                var list = Category.ParentCategories.Select(c => new CategoryTreeWalker
                {
                    Category = Category,
                    CurrentCategory = c,
                    ParentCategory = c.Parent,
                    ParentNodeTypeAlias = c.ParentNodeTypeAlias,
                    Parent = this
                }).ToList();
                if (ParentCategory != null)
                {
                    list.Add(new CategoryTreeWalker
                    {
                        Category = Category,
                        CurrentCategory = ParentCategory,
                        ParentCategory = ParentCategory.Parent,
                        ParentNodeTypeAlias = ParentCategory.ParentNodeTypeAlias,
                        Parent = this
                    });
                }
                return list;
