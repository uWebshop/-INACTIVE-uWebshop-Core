...
namespace uWebshop.Domain
{
	...
	[Serializable]
	[ContentType(ParentContentType = typeof(ShippingProviderSectionContentType), Name = "Shipping Provider Method", Description = "#ShippingProviderMethodDescription", Alias = "uwbsShippingProviderMethod", IconClass = IconClass.link, Icon = ContentIcon.MagnetSmall, Thumbnail = ContentThumbnail.Folder)]
	public class ShippingProviderMethod : uWebshopEntity
	{
		/// <summary>
		/// The node alias
		/// </summary>
		...
		public int PriceInCents
		{
			get
			{
				if (ShippingProviderUpdateService != null)
				{
					ShippingProviderUpdateService.Update(this, OrderHelper.GetOrder()); // todo: check if current order is always correct
				}
				return _priceInCents;
			}
			set { _priceInCents = value; }
		}
		...
		{
			get { return Businesslogic.Price.CreateSimplePrice(PriceInCents, IO.Container.Resolve<ISettingsService>().IncludingVat, Vat, StoreHelper.CurrentLocalization); }
		}
		...
		/// </value>
		[XmlIgnore]
		public IShippingProviderUpdateService ShippingProviderUpdateService { get; set; }
		...
		internal void UpdatePriceFromCustomShippingProvider(OrderInfo order)
		{
			if (ShippingProviderUpdateService != null)
				ShippingProviderUpdateService.Update(this, order);

			order.ShippingProviderAmountInCents = PriceInCents;
		}
	}
}