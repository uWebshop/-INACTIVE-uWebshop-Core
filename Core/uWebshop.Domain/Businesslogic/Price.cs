...
namespace uWebshop.Domain.Businesslogic
{
	...
	internal class PriceOrderline : SummedPrice
	{
		...
		public PriceOrderline(IDiscountedRangedPrice price, bool includingVat, decimal vat, ILocalization localization
			, IVatCalculationStrategy vatCalculationStrategy, int lineItemCount) : base(new []{price}, includingVat, vat, localization,vatCalculationStrategy)
		{
	...
	internal sealed class Price : IDiscountedRangedPrice, IPrice
	{
		...
		private Price(int price, bool includingVat, decimal vat, ILocalization localization) : this(price, localization)
		{
		...
		public int ValueInCents
		{
			get
			{
				return _calculateVat(_applyDiscount(_currentOriginalPrice));
			} 
		}
		...
		public IDiscountedPrice Ranged
		{
			...
			{
				_ranged = true;
				_currentOriginalPrice = DoTheRangeThing();
				return this;
			}
		}
		public IRangedPrice BeforeDiscount
		...
		private int DoTheRangeThing()
		{
			...
			if (order != null && _orderCountCalculation != null)
			{
				var totalCount = _orderCountCalculation(order);
					// order.OrderLines.Where(line => line.ProductInfo.Id == _productVariantVariant.Id).Sum(line => line.ProductInfo.ItemCount.GetValueOrDefault(1));
